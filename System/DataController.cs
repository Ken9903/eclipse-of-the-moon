...
public class DataController : MonoBehaviour
{
    ...
    public void LoadGameData()
    {
        ...
        skillManager.EarnedAble.Clear();
        skillManager.skillInit();
        for (int i = 0; i < gameData.EarnedAbleObj.Count; i++)
        {
            skillManager.EarnedAble.Add(Resources.Load<GameObject>(gameData.EarnedAbleObj[i]), gameData.EarnedAbleCost[i]);
        }
    ...
    public void SaveGameData()
    {
        ...
        //스킬매니저
        SkillManager skillManager = GameObject.Find("SkillManager").GetComponent<SkillManager>();
        gameData.EarnedAbleObj.Clear();
        gameData.EarnedAbleCost.Clear();
        foreach (KeyValuePair<GameObject, int> item in skillManager.EarnedAble)
        {
            gameData.EarnedAbleObj.Add(item.Key.name);
            gameData.EarnedAbleCost.Add(item.Value);
        }
