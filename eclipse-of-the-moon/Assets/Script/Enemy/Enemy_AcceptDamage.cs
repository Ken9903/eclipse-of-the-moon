...
public class Enemy_AcceptDamage : MonoBehaviour
{
    ...
    public void Accept_Damage_Oneshot(int damage) // 미세조정 필요할때를 위해 분리함 아직은 분리의 의미가 없음.
    {
        ...
        ShowDamageUi(damage);
            ...
            if (!anim.GetCurrentAnimatorStateInfo(0).IsName("Attack"))
            {
                ...
                StartCoroutine(AffectTriggerOnOff()); //다른 몹까지 끌어들이는 기능
            }
    ...
    {
        if(enemyStatus.hp <= 0 && dead == false) // dead는 한번만 실행 되게 하도록 하는 장치. 경험치 중복 획득 방지.
        {
            dead = true;
            GiveExpToPlayer(); // 플레이어에게 경험치 부여
