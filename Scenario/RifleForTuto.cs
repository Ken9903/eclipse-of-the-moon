...
public class RifleForTuto : MonoBehaviour
{
    ...
    public float destroyMagDelay = 0.4f;


    private bool reload = true;  //단발용 변수

    public int passive1RandomPercent; //패시브1 발동 확률
    public int passive2RandomPercent; //패시브2 발동 확률
    public int passive3RandomPercent; //패시브3 발동 확률
    ...
    void Update()
    {
        if (magEquiped)
        {
            ...
            {
                if (triggerTargetR >= 0.9 && bullet_type == "basic" && reload)
                {
                    Shot();
                    reload = false;
                }

                if (triggerTargetR >= 0.9 && bullet_type == "skill" && reload)
                {
    ...
    void Delete_Mag()
    {
        ...
        Destroy(current_mag, destroyMagDelay);
    }
